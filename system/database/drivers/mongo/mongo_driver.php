...
class CI_DB_mongo_driver
{
	...
	{
		$this->init_collection($collection);
	...
	public function like($key, $value = null)
	{
		if (is_array($key)) {
			foreach ($key as $k => $v) {
				$this->wheres[$k] = new MongoRegex("/$v/");
			}

		} else {
			$this->wheres[$key] = new MongoRegex("/$value/");
		}
	...
	public function not_like($key, $value = null)
	{
		if (is_array($key)) {
			foreach ($key as $k => $v) {
				$this->wheres[$k]['$nin'][] = new MongoRegex("/$v/");
			}

		} else {
			$this->wheres[$key]['$nin'][] = new MongoRegex("/$value/");
		}
	 ...
	 * @return array
	 */
	public function distict($field = '', $collection = null)
	{
		if (! $this->collection) {
			$this->init_collection($collection);
		}
		...
		try {
			return $this->collection->distinct($key, $this->wheres);
	...
	public function count_all_results($collection = null)
	{
		...
		$count = $this->get($collection)->num_rows();
